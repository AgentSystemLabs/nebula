...
Forty-nine keys. **Overlay** is the SETTINGS OVERLAY tab whose row edits the key; `—` means the key
...
- **Settings live in one JSON file** (`config.json`, beside the database, with `config.local.json` over
  ...
  both), and whether `a` asks before archiving one (`confirm_on_archive`, off unless you turn it
