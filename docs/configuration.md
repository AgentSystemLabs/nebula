...
| `launcher_view` | bool | `false` | Experimental | The LAUNCHER VIEW: nebula built around the prompt instead of the tree. It opens on the QUICK PROMPT, already focused, aimed at the selected project and at a fresh worktree, with the harness, model and effort the Agents tab defaults name — `^P` in it picks the project by typing through every one on the machine, another workspace's included, without moving the screen there: a launch into a project other than the one on screen runs in the background, and only the footer says where it went. `^O` picks the model, `Tab` the harness, `^N` flips between a fresh worktree and the project's checkout (the choice sticks for the next box). The three panels and the Workspaces bar give way to a GRID of session cards, over a pane along the bottom that reads the card under the cursor, under a `nebula / workspace / project / sessions` header that counts them, with a STATUS TALLY of dots beside the trail — one per state with a card in it, carrying its count and no word at all: red waiting on you, blue an unread finish, yellow mid-turn, purple landed, left out where a state is empty (every crumb is a button that opens what its word names — `nebula` the machine's workspaces, a workspace its projects — where `Esc` goes): every unarchived session of the open workspace, most recently touched first — the order the Sessions panel lists them in, working and blocked sessions at the front — up to four cards a row (fewer as the terminal narrows), each its name and status, where it runs and with what (`↳ feat · claude opus` — the checkout, not the project, which the header already names), its pull request (`↗ #42 title  ready`, colored as the PR rows are) and the last thing it was asked, wrapped over three rows. `h`/`j`/`k`/`l` or a click walk the cards — the wheel is ignored over them, so a stray notch cannot swap the pane out from under you — swapping the pane onto the one you land on and reading it as it arrives; a click into the pane types into that session where it stands and `^q` comes back to the cards. Enter (or `Tab`, `^→`, or a double-click) steps down into that pane with its input locked, the grid still up over it and `^q` handing the keys back to the cards; `z` takes the session full-screen, `‹ sessions` in its header and `^q` coming back to the grid; `p` opens the box again, and every other key keeps its panel meaning on the card under the cursor. Off by default: it replaces the whole layout. See [Sessions](sessions.md#the-launcher-view). |
