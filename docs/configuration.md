...
| `focus_tint` | bool | `true` | Appearance | The FOCUSED PANEL TINT: a faint accent wash behind whatever keys land in — the card under the cursor while the grid has them, the session pane while it does. The wash is the theme's accent taken down to a near-black, not a gray, so text on it keeps its contrast. Off leaves every cell on the terminal's own background, so a transparency or image configured in the terminal shows through the whole window instead of stopping at the focused panel. |
