...
| `quick_prompt_new_worktree` | bool | `false` | Agents | QUICK PROMPT NEW WORKTREE: whether each new QUICK PROMPT starts aimed at a fresh worktree (a random branch off the **Worktree base branch**) instead of the project's root branch. `Ctrl+N` in the box flips only that box; the next one starts from this again. Off by default, so `p` starts on the root branch — never in the checkout of the card under the cursor, whose session takes more work as a follow-up (`Space`). |
