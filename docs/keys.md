...
| Terminal / any OVERLAY | `Ctrl+q` | the HARDWIRED UNLOCK, and the one binding that is more than a hotkey. From a locked TERMINAL PANE it goes back to the panels (also expanding collapsed sidebars). With any OVERLAY open it force-closes that overlay in a single press from any nesting depth — a typed filter, an open submenu, a live hotkey capture, the preset editor stacked over its list, the editor modal — landing you on the panels every time. It stays wired in on top of whatever the KEYMAP says, so no rebind, nested modal or half-typed field can ever trap you. `Ctrl+Shift+H` (KITTY PROTOCOL only), `Ctrl+]`, `Ctrl+Esc` and `Ctrl+←` are the other four defaults for leaving the pane, for terminals that eat one of them — but those four are ordinary rebindable bindings, and only `Ctrl+q` closes an OVERLAY |
