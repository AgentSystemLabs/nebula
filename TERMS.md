...
| **PROJECTS PANEL** | Optional left sidebar listing the OPEN WORKSPACE's PROJECTS. Its header reads `PROJECTS` while the WORKSPACES BAR is shown and the workspace's name (upper-cased) when the bar is hidden. | "projects list", "projects column", "projects", "projects worktrees and sessions lists" | `ui.rs::draw_projects` · `Focus::Projects` · `Config::hide_projects` · `Shift+P` |
| **WORKTREES PANEL** | Optional sidebar listing the selected PROJECT's WORKTREES (PINNED / UNPINNED) plus the OPEN PRS GROUP. | "worktrees column", "worktrees list", "worktrees row" | `ui.rs::draw_worktrees` · `Focus::Worktrees` · `Config::hide_worktrees` · `Shift+B` |
| **SESSIONS PANEL** | Always-visible sidebar listing the selected WORKTREE's SESSIONS in SESSION GROUPS, plus LINKS. | "sessions list", "sessions column", "session list", "sessions", "recent list" | `ui.rs::draw_sessions` · `Focus::Sessions` |
