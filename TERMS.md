...
| **PROTOCOL VERSION** | `PROTOCOL_VERSION` (30) exchanged in the HANDSHAKE. Frames are positional msgpack, so any new field on a shared struct bumps it. Two branches that each bump it merge to the *same* number — diff it against `origin/main` before a release commit. | "protocol", "v26/v27/v28/v29/v30" | `nebula-core/src/protocol.rs::PROTOCOL_VERSION` |
...
| **AGENT ENV** | `NEBULA_AGENT_ID` / `NEBULA_API_URL` / `NEBULA_API_TOKEN` set on AGENT PTYs (and scrubbed from TERMINAL SESSIONS) — how hooks and `nebula rename` / `nebula worktree` know which row they are. | — | `registry.rs::scrubbed_env_names` |
...
| **WORKTREES PANEL** | Optional middle column: the selected PROJECT's WORKTREES (PINNED / UNPINNED) plus the PROJECT OPEN PRS GROUP. | "worktrees column", "worktrees list", "worktrees row" | `ui.rs::draw_worktrees` · `Focus::Worktrees` · `Config::hide_worktrees` · `Shift+B` |
| **SESSIONS PANEL** | Always-visible third column: the selected WORKTREE's SESSIONS in SESSION GROUPS, plus the WORKTREE OPEN PRS GROUP. | "sessions list", "sessions column", "session list", "sessions", "recent list" | `ui.rs::draw_sessions` · `Focus::Sessions` |
...
| **FOCUS TINT** | The faint accent background over the focused PANEL (truecolor). Setting `focus_tint`. | "focused-panel tint", "focus terminal setting", "lightly colored (like 10% opacity) theme color" | `ui.rs::draw_focus_tint` |
...
| **FINISHED** | The turn completed. Draws violet while UNSEEN and green (`ok`) once read. | "done", "green", "finished" | `AgentStatus::Finished` |
...
| **IDLE PROMPT** | Claude's `Notification{idle_prompt}` hook event, which un-sticks a RUNNING turn that never sent Stop. A cancel suppresses it too. | "idle_prompt", "idle notification" | `status.rs::HookEvent::Notification` |
...
| **PANEL WALK** | Moving FOCUS with FOCUS NEXT / FOCUS PREV (Tab / Shift+Tab, `Ctrl+Shift+L` / `Ctrl+Shift+H`). It skips hidden panels; forward stops at the TERMINAL PANE and locks it (LOCKED PANE), while backward stops at the WORKSPACES BAR when shown or the first visible sidebar. Neither direction cycles or wraps. | "the walk", "the nav", "cycle the nav", "control shift h or l", "tab through the panels", "navigation keys to toggle through", "h and l should be for left and right" | `event_loop.rs::walk_focus_forward` / `walk_focus_back` · `App::next_visible_focus` · `Tab` / `Shift+Tab` |
| **WALK EDGE** | The end of the row where a single `h`/`l` (or ←/→) is a no-op: the SESSIONS PANEL (or an unlocked TERMINAL PANE) going right, the first visible sidebar going left. A DOUBLE TAP there jumps the boundary. | "locked layer", "the edge", "the end of the row", "blocked boundary" | `event_loop.rs::Action::FocusLeft` / `FocusRight` |
| **DOUBLE TAP** | Two presses of the same `h`/`l` (or ←/→) within `DOUBLE_TAP` (400 ms) at a WALK EDGE: `l`,`l` at the SESSIONS PANEL enters and locks the pane like FOCUS NEXT, while `h`,`h` at the first visible sidebar steps up into the WORKSPACES BAR when shown. The first press stays put and flashes "`l` again: enter pane"; any other key in between, or a slower second press, breaks the pair. | "double tap", "double tap h or l", "jump over that blocked boundary", "second press" | `event_loop.rs::double_tapped` · `DOUBLE_TAP` · `app.rs::App::edge_tap` |
...
| **MOVE UP / MOVE DOWN** | Move the selection inside the focused PANEL. | "j and k" | `Action::MoveUp` / `MoveDown` · `k`/`↑`, `j`/`↓` |
...
| **LOCKED PANE** | The TERMINAL PANE with `term_locked` set: every key is forwarded raw to the PTY except the ESCAPE HATCHES. Reached by ACTIVATE on a session, the forward PANEL WALK, a DOUBLE TAP, or ZOOM. The chip reads `INPUT`. | "locked terminal", "in the session", "typing at the agent", "input mode" | `app.rs::App::term_locked` · `event_loop.rs::enter_terminal_pane` |
...
| **CONTEXT MENU** | `m` / right-click: the row menu of MENU ACTIONS for whatever is selected — attach, restart, rename, archive, pin, delete, **Attach cloud session**, **Send to cloud session**, workspace verbs. `→` opens the MODEL / EFFORT submenus. | "menu", "right click menu", "the m menu", "m menu" | `app.rs::Overlay::Menu`, `MenuAction` · `m` |
...
| **NEW SESSION PICKER** | The menu `n` opens in the SESSIONS PANEL: Claude / Codex / Cursor / Terminal (shell); `→` drills into MODEL and EFFORT; `Tab` on Claude toggles CLOUD LAUNCH. | "session picker", "new session menu", "the agent picker", "new session harness selection modal", "modal that let's me pick codex or claude" | `app.rs::MenuAction::NewAgentOfKind` · `n` |
| **CLOUD TASK EDITOR** | The wrapped multi-line editor (`Shift+Enter` / `Ctrl+J` newline) for a CLOUD LAUNCH task or a CLOUD MESSAGE. | "cloud prompt", "the task box", "dialog prompt so a user can type their prompt", "word wraps" | `app.rs::PromptKind::ClaudeCloudTask` / `CloudMessage` |
...
| **SETTINGS OVERLAY** | `s`: tabs General / Sessions / Appearance / Agents / Hotkeys over CONFIG.JSON; `R` resets everything after a CONFIRM DIALOG. Reopening within a minute lands back on the row you left. | "settings", "the settings modal", "preferences", "settings modal", "the settings", "reset to default" | `app.rs::Overlay::Settings`, `SettingsView` · `config.rs::SETTINGS_TABS` · `s` |
...
| **CLAUDE HOOK DIALECT** | PascalCase events in `.claude/settings.local.json`, plus `permissions.allow` rules for `Bash(nebula rename:*)` and `Bash(nebula worktree:*)`. Only `UserPromptSubmit`'s command keeps stdout, so injected context reaches the model. | "claude hooks" | `hooks/installer.rs::CLAUDE_EVENTS`, `CLAUDE_ALLOW_RULES` |
...
| **AUTO-TITLE** | A session left on its default name (`agent-N`) gets the AUTO-TITLE INSTRUCTION injected on its first `UserPromptSubmit`; the agent runs `nebula rename <3-4 words>` once; the daemon applies it only while `auto_title_pending` is set, so a user RENAME always wins. | "auto title", "name themselves", "the title hook", "automatically rename the session", "title between 3-4 words" | `hooks/mod.rs::auto_title_injection` · `registry.rs::auto_rename_agent` |
...
| **WORKTREE GUIDANCE** | The `--append-system-prompt` every Claude launch gets: run `nebula worktree <name>` instead of `EnterWorktree` when asked for a worktree. | "the worktree instruction", "skill + system prompt or something" | `registry.rs::CLAUDE_WORKTREE_GUIDANCE` |
...
| **SETTING** | One CONFIG.JSON key: `theme`, `animations`, `focus_tint`, `show_workspaces` (the WORKSPACES BAR), `hide_projects` (the PROJECTS PANEL), `hide_worktrees` (the WORKTREES PANEL), `editor`, `claude_model`/`claude_effort`, `codex_model`/`codex_effort`, `recent_window`, `session_idle_timeout`, `skip_session_naming`, `palette_enter_attaches`, `git_init_on_create`, `keybindings`; daemon-only `prewarm_agents`, `prewarm_sessions`. | "setting", "option" | `config.rs::Config` |
| **RECENT WINDOW** | How long an unpinned SESSION stays in the RECENT group (`off`/`5m`/…/`24h`, default 30m). | "recent" | `config.rs::RECENT_WINDOWS` |
...
| **RELEASE SKILL** | `/release`: verify green, commit, bump `[workspace.package].version`, tag, push, replace the GitHub notes with a real changelog. Triggered by "commit push release". | "commit push release", "cut a release", "ship it", "do a release", "commit and push and release", "make another release" | `.claude/skills/release/SKILL.md` |
...
| **SELF-IMPROVING LOOP** | The per-task cycle every agent runs: read the MEMORY LOG and `TERMS.md` → PROMPT DADDY → do the work → NEBULA-MEMORY SKILL → PROJECT TERMS → OUTPUT DOCTOR shapes the reply, so each task leaves the next one better grounded. Its two files are also where a long-lived branch re-conflicts with `main` on every merge. | "the self improving loop", "the self improving look" | `CLAUDE.md` § "Project memory and vocabulary" · `AGENTS.md` |
...
| **PROJECT DIVIDERS** | Labelled separators between PROJECTS (`-` key, `divider_*` columns). Removed end to end; MIGRATION 18 drops the columns (old migrations must keep spelling them). MOVE PROJECT stayed as a plain reorder. | 2026-08-25 |
...
| **TOGGLE PROJECTS PANEL** | The independent show/hide action for the PROJECTS PANEL; released width goes to the TERMINAL PANE and restoring the panel does not move FOCUS. | 2026-08-28 prompt ("toggle the PROJECTS column") · 2026-08-28 MEMORY "Independent PROJECTS PANEL And WORKTREES PANEL Visibility" | `keymap.rs::Action::ToggleProjects` · `Config::hide_projects` · `Shift+P` |
| **TOGGLE WORKTREES PANEL** | The independent show/hide action for the WORKTREES PANEL; released width goes to the TERMINAL PANE and restoring the panel does not move FOCUS. | 2026-08-28 prompt ("toggle WORKTREES column separate") · 2026-08-28 MEMORY "Independent PROJECTS PANEL And WORKTREES PANEL Visibility" | `keymap.rs::Action::ToggleWorktrees` · `Config::hide_worktrees` · `Shift+B` |
...
| They say | They mean |
|---|---|
| "top nav", "top bar", "workspaces top bar", "header", "workspace header tabs", "the tab bar" | WORKSPACES BAR |
...
| "locked layer", "blocked boundary" | WALK EDGE — *not* LOCKED PANE (in a locked pane `h`/`l` go to the agent) |
| "double tap", "double tap h or l", "second press", "jump over that blocked boundary" | DOUBLE TAP |
...
| "unknown agents", "warm spares", "sub agents" (mistaken) | PREWARM POOL / WARM SPARE |
...
| "session picker", "new session menu", "the agent picker" | NEW SESSION PICKER |
...
| "commit push release", "cut a release", "ship it", "make a release", "make another release" | RELEASE SKILL |
...
| "the self improving loop", "the self improving look" | SELF-IMPROVING LOOP |
