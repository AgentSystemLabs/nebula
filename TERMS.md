...
| **STATUS DOT** | The colored ● before a row name: gray FRESH, yellow RUNNING, violet FINISHED+UNSEEN, green FINISHED read, red NEEDS FEEDBACK, magenta TERMINATED, ○ DISCONNECTED. | "status dot", "the dot", "the color" | `ui.rs::status_dot(status, unseen, th)` · `status_color` (the color alone; the PILL ROW rail, the PROJECT `▌` and the TAB UNDERLINE take it) |
...
| **CURSOR MODEL FAMILY** | The model half of a Cursor MODEL / EFFORT pick (`claude-opus-5-thinking`, `gpt-5.6-sol`, `auto`): the catalogue id minus its effort suffix, which `cursor-agent --model` gets back joined with a `-`. | 2026-08-28 MEMORY "`cursor-agent --model` Takes A `<family>-<effort>` Id…" | `nebula-tui/src/cursor_catalogue.rs::split_id` |
| **CURSOR CATALOGUE** | The Cursor model list the TUI hands to every MODEL / EFFORT surface: a built-in seed of catalogue ids unioned with `cursor-agent --list-models`, cached in `<data dir>/cursor_models.json` for a day and refreshed on a background thread at TUI startup. | 2026-08-28 MEMORY "`cursor-agent --model` Takes A `<family>-<effort>` Id…" (follow-up) | `nebula-tui/src/cursor_catalogue.rs::bootstrap` · `cursor_models.json` |
| **MAKE PRUNE** | `make prune` / `scripts/prune-target.py`: the TARGET PRUNE step of MAKE CYCLE — keeps the newest `KEEP=3` hashes of every crate in `target/<profile>/{deps,.fingerprint,build,incremental}` and deletes the rest, holding cargo's `.cargo-lock` so it never deletes under a running build; `--dry-run` reports only. Born when ten days of release bumps × `cargo build`/`test`/`check` variants left 372k `.o` files (41GB) in `target/debug/deps`. | 2026-08-29 prompt ("update the make cycle to automatically clean up old stuff") · 2026-08-29 MEMORY "`make prune` Drops Stale target/ Hashes" | `Makefile::prune` · `scripts/prune-target.py` |
| **ATTENTION ORDER** | The PALETTE's row order before a query — NEEDS FEEDBACK sessions, then RUNNING, then FINISHED + UNSEEN, then every other row in RECENCY ORDER with never-run rows in tree order and ARCHIVED last — and its tiebreak between equal FUZZY MATCH scores. | 2026-08-29 prompt ("sorted to the top by default … everything else should sort under those rules") · 2026-08-29 MEMORY "The PALETTE Sorts By Attention, Then RECENCY ORDER, Before You Type" | `palette.rs::PaletteTier` · `palette.rs::attention_rank` |
