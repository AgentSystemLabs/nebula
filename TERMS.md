...
`project-terms` skill (`.claude/skills/project-terms/SKILL.md`), which prunes with
`.claude/memory/terms_check.py` (`make terms-check`: dead TERMS, merge pairs, stale pointers, overdue
candidates — `--merge`, `--retire`, `--prune` rewrite every reference); history and gotchas live in
...
| **SESSION GROUPS** | The SESSIONS PANEL's headers: the AGENTS come first as one header-less list in RECENCY ORDER, then TERMINALS, OPEN PRS, ARCHIVED (hidden until `Shift+A`, see SESSIONS PANEL). The WORKTREES PANEL has only its checkouts and OPEN PRS. PINNED / RECENT / UNPINNED were retired with PIN and the RECENT WINDOW. | "the groups", "the recent label", "archived list" | `ui.rs::draw_sessions` |
...
| **TUI** | The ratatui client (bare `nebula`) that attaches to the DAEMON over the DAEMON SOCKET. Quitting it (`q` / `Ctrl+C`, behind a CONFIRM DIALOG since 2026-08-29; a second `Ctrl+C` goes straight through) kills nothing. | "the ui", "nebula", "the app", "quit", "q", "closed out" | `crates/nebula-tui` · `keymap.rs::Action::Quit` |
| **DAEMON SOCKET** | `<RUNTIME DIR>/daemon.sock`, mode 0700 — the unix socket the TUI and one-shot CLI clients connect to. | "the socket", "pidfile lock", "pidfile", "buildstamp" | `nebula-core/src/paths.rs::socket_path` |
...
| **DATA DIR** | Where the SQLITE STORE, CONFIG.JSON, REVIEWED MARK and the SSH HOSTS file live: `$NEBULA_DATA_DIR` or the platform app-support dir (`~/.local/share/nebula`, `~/Library/Application Support/dev.nebula.nebula`). | "data dir" | `paths.rs::data_dir` · `NEBULA_DATA_DIR` |
...
| **PROTOCOL VERSION** | `PROTOCOL_VERSION` (33) exchanged in the VERSION SKEW. Frames are positional msgpack, so any new field on a shared struct bumps it. Two branches that each bump it merge to the *same* number — diff it against `origin/main` before a release commit. | "protocol", "v26/v27/v28/v29/v30/v32" | `nebula-core/src/protocol.rs::PROTOCOL_VERSION` |
| **VERSION SKEW** | A DAEMON and a client built from different PROTOCOL VERSIONS, caught by the handshake (`Hello{protocol_version}` → `HelloOk{daemon_pid}` or `Incompatible`, then `Subscribe` → `Snapshot`). The client's message names both binaries and says which side is older: `make install` when the client is, `nebula kill` when the daemon is. | "daemon speaks protocol v26, this client v24", "the hook still seems to fail", "handshake" | `nebula-tui/src/ipc.rs::version_skew_message` |
...
| **IPC CODEC** | Length-prefixed MessagePack frames over the DAEMON SOCKET, 4 MiB max. | "msgpack", "rmp", "client request" | `nebula-core/src/codec.rs` |
...
| **PTY SESSION** | The DAEMON-owned PTY child for one SESSION: reader thread, output coalescing, SIGHUP-then-SIGKILL on kill, plus the PROGRESS SCANNER, KITTY PROTOCOL and CLOUD SCANNER on its stream. | "the pty" | `nebula-daemon/src/pty/mod.rs::PtySession` |
...
| **AGENT ENV** | `NEBULA_AGENT_ID` / `NEBULA_API_URL` / `NEBULA_API_TOKEN` set on AGENT PTYs (and scrubbed from TERMINAL SESSIONS) — how hooks and `nebula rename` / `nebula worktree` / `nebula spawn` know which row they are. | —, "env guard" | `nebula-core/src/env.rs::AGENT_ID` |
...
| **TAB UNDERLINE** | The `▀` under the open WORKSPACE TAB, in the tab's ROLLUP STATUS DOT color (the accent before 2026-08-28), flush with the tab's fill (a `━` left a half-cell gap). | "the bottom bar", "the underline", "green underline", "gap under the tab" | `ui.rs::draw_workspaces_bar` · `selection_mark` |
...
| **SESSIONS PANEL** | Always-visible third column: the selected WORKTREE's SESSIONS in SESSION GROUPS, plus the WORKTREE OPEN PRS GROUP. | "sessions list", "sessions column", "session list", "sessions", "recent list", "focused on the session", "archive", "archived" | `ui.rs::draw_sessions` · `Focus::Sessions` |
| **RECENCY ORDER** | How every list column orders itself: PROJECTS, WORKTREES and live SESSIONS sit most-recently-interacted first, a RUNNING session counting as now, never-run rows keeping tree order at the bottom. There is no manual reorder (MOVE PROJECT is retired) and no PIN (retired); the AGO BADGE shows the stamp. | "recent to top", "order by last interaction", "goes to top of list", "always just move recent to top", "recent at the top", "time stamps", "most recently used" | `app.rs::Recency` · `last_interaction_ms` · `project_rows` · `visible_worktrees` · `visible_sessions` |
...
| **PILL ROW** | A sidebar row drawn as a 3-row pill (pad, text, pad) on a 2-row stride, with a rail when selected: the row's STATUS DOT color in the focused PANEL (the accent on a row with no dot — PR ROW, terminal, link — dim unfocused). Hit-tested over its whole drawn height. | "row", "pill", "clickable zone", "row itself", "black bars top and bottom", "the underline" / "green underline" (the rail) | `ui.rs::PILL_H`, `pill_hit_height` · `render_pill(.., mark)` |
...
| TERM | What it is | Also called | Where · key |
|---|---|---|---|
| **AGENT STATUS** | The DAEMON-owned state of an AGENT: FRESH, RUNNING, FINISHED, NEEDS FEEDBACK, terminated, disconnected. Driven by the STATUS MACHINE. | "status", "state", "boot sweep", "disconnected", "terminated", "died" | `nebula-core/src/entities.rs::AgentStatus` |
...
| **NEEDS FEEDBACK** | Red ● — a permission prompt or question is waiting on you. Cursor agents never reach it (no permission hook, `--force`). | "red", "needs feedback", "waiting on me", "permission prompt", "awaiting-feedback", "needing user input" | `AgentStatus::NeedsFeedback` |
...
| **STATUS DOT** | The colored ● before a row name: gray FRESH, yellow RUNNING, violet FINISHED+UNSEEN, green FINISHED read, red NEEDS FEEDBACK, magenta AGENT STATUS, ○ AGENT STATUS. | "status dot", "the dot", "the color" | `ui.rs::status_dot(status, unseen, th)` · `status_color` (the color alone; the PILL ROW rail, the PROJECT `▌` and the TAB UNDERLINE take it) |
...
| **LOCKED PANE** | The TERMINAL PANE with `term_locked` set: every key is forwarded raw to the PTY except the ESCAPE HATCHES. Reached by ACTIVATE on a session, the forward PANEL WALK, a DOUBLE TAP, or zoom. The chip reads `INPUT`. | "locked terminal", "in the session", "typing at the agent", "input mode", "zoom", "full screen" | `app.rs::App::term_locked` · `event_loop/focus_walk.rs::enter_terminal_pane` |
...
| **HARDWIRED UNLOCK** | `Ctrl+Q` always unlocks (and force-closes the FILE FINDER) no matter what is bound — so a rebind can never trap you in a session. | "ctrl q", "the final escape hatch" | `event_loop.rs::HARDWIRED_UNLOCK` · `Ctrl+Q` |
| **KEYMAP** | The rebindable table of action ids (`focus_left`, `git_diff` — the keys in `"keybindings"`; unknown ids are ignored) → chords (`ctrl+shift+f`, `cmd+1`, comma-separated), with defaults in code and overrides in CONFIG.JSON, each binding live in one scope: `Global` (panels) or `Terminal` (the LOCKED PANE). Edited on the HOTKEYS TAB. | "keybindings", "hotkeys", "the keys", "chord", "shortcut", "hotkey", "action id", "action", "key scope", "scope" | `nebula-tui/src/keymap.rs::Keymap` · `"keybindings"` |
| **HOST REACH WARNING** | The HOTKEYS TAB's bind-time note that the host terminal will probably eat a KEYMAP: any `⌘`, `^⇧` without the KITTY PROTOCOL, `^←` on stock macOS. | "won't survive the trip", "command + p pastes the pi character", "cmd never reaches the pty" | `keymap.rs::host_warning` |
| **KITTY PROTOCOL** | The kitty keyboard protocol (`CSI … u`) that lets `Ctrl+Shift+…` chords reach nebula at all — Ghostty speaks it, Terminal.app does not. The kitty scanner answers the child's queries. | "kitty", "csi u", "kitty scanner" | `nebula-daemon/src/pty/kitty.rs::KittyScanner` |
| **HOTKEY CAPTURE** | Pressing a new KEYMAP on a HOTKEYS TAB row; a duplicate shows `already X — Enter to move it here`. | "rebind" | `app.rs::HotkeyCapture` · `event_loop.rs::bind_hotkey` |
...
| **CONTEXT MENU** | `m` / right-click: the row menu of MENU ACTIONS for whatever is selected — attach, restart, rename, archive, delete, **Attach cloud session**, **Send to cloud session**, workspace verbs. `→` opens the MODEL / EFFORT submenus. | "menu", "right click menu", "the m menu", "m menu", "restart agent", "restart" | `app.rs::Overlay::Menu`, `MenuAction` · `m` |
| **CONFIRM DIALOG** | A yes/no gate (`y`/`Enter` vs `Esc`/`n`) before a pending action — delete, remove, reset settings, quit. Sized to its longest line (min 52 cols), no wrapping. | "confirmation", "are you sure", "pending action" | `app.rs::Overlay::Confirm`, `PendingAction` |
...
| **NEW SESSION PICKER** | The menu `n` opens in the SESSIONS PANEL: Claude / Codex / Cursor, minus any AGENT KIND switched off on the SETTINGS OVERLAY's Agents tab (absent, not greyed; no Terminal row since 2026-08-28 — NEW TERMINAL is `t`); `→` drills into MODEL and EFFORT submenus where typing filters the rows (fuzzy, best first; ↑/↓ move, Esc clears); `Tab` on Claude toggles CLOUD LAUNCH. | "session picker", "new session menu", "the agent picker", "new session harness selection modal", "modal that let's me pick codex or claude", "new session modal", "harness picker modal" | `app.rs::MenuAction::NewAgentOfKind` · `n` |
...
| **AGENT PRESET** | A saved launch definition — name, AGENT KIND, MODEL / EFFORT, optional prefix and postfix text — listed by `e` in the SESSIONS PANEL (the list modal), kept in `agent_presets.json` beside CONFIG.JSON, and launched with one task whose prefix + task + postfix becomes the STARTING PROMPT; the row it creates is an ordinary AGENT. Edited in the PRESET EDITOR. | "agent modal", "agent preset modal", "agent definition", "pre-configured agent definition", "pre-configured agent", "prefix and postfix prompts", "sandwich the request" | `nebula-tui/src/agent_presets.rs::AgentPreset` · `preset_overlays.rs::AgentPresetsView` · `Action::AgentPresets` · `e` |
| **PRESET EDITOR** | The form behind the AGENT PRESET list's `a` / `e`: Name, Harness, Model, Effort rows (Tab / ↑↓ between fields, ←/→ cycling a choice or typing to jump to the best fuzzy match; Effort reads `n/a` and is skipped when the kind + model pair has no effort list) over two multi-line Prefix / Postfix boxes; Enter saves, Esc backs out to the list. | "the preset form", "presets modal" | `preset_overlays.rs::AgentPresetEditor`, `PresetField` |
| **TYPE-AHEAD** | Typing letters into a list to narrow it — the MODEL / EFFORT submenus (`ContextMenu.filter`) and the PRESET EDITOR's choice rows (`AgentPresetEditor.filter`), both over `fuzzy::rank`; ↑/↓ move, Esc clears. | "type ahead", "type agead" | `app.rs::MenuFilter` · `preset_overlays.rs::AgentPresetEditor::type_filter` |
...
| **HOTKEYS TAB** | The SETTINGS OVERLAY tab that lists every KEYMAP, what it answers to, and rebinds it (HOTKEY CAPTURE, HOST REACH WARNING). | "hotkeys", "keybindings tab", "customize ANY HOTKEY" | `config.rs::SETTINGS_TABS` |
| **AGENTS TAB** | The SETTINGS OVERLAY tab holding, per AGENT KIND, the `enabled` toggle and the MODEL / EFFORT defaults (every AGENT KIND gets model and effort rows; Cursor's effort row reads `n/a` until a model family with effort variants is chosen). Switching a harness off hides it from the NEW SESSION PICKER and the PR SESSION launch; the last one on is refused with a warning. | "agents tab" | `nebula-tui/src/config.rs::SETTINGS_TABS` ("Agents") · `Config::enabled_kinds` |
...
| **REVIEWED MARK** | The ✓ that sinks a file to the bottom of the DIFF VIEWER — nebula-side only, scoped to worktree + HEAD, cleared when the file changes again. Stored in reviewed.json. | "reviewed", "the tick", "mark as read", "reviewed.json" | `nebula-tui/src/review.rs` · `Ctrl+r` |
| **PALETTE** | `/`: the ` Jump to ` fuzzy search across every WORKSPACE, PROJECT, WORKTREE, SESSION and open PR, in *every* workspace (`workspace/project/branch/session` paths); until you type, the rows sit in attention order — NEEDS FEEDBACK, RUNNING, then FINISHED + UNSEEN sessions, then everything else in RECENCY ORDER, ARCHIVED last — and that order breaks score ties once you do. `Ctrl+o` opens, `Ctrl+f` only lands the selection; picking a hit elsewhere switches workspace on the way. | "fuzzy jump", "slash search", "jump to", "the search", "/ fuzzy finder", "the / fuzzy find", "fuzzy / finder", "attention order", "sorted to the top by default", "needing user input … sorted to the top" | `app.rs::Overlay::Palette` · `nebula-tui/src/palette.rs::Palette` · `nebula-tui/src/fuzzy.rs` · `/` |
...
| **FILE FINDER** | `f`: fuzzy file picker over the WORKTREE (`Enter` opens the file, `Ctrl+y` copies the path); `Shift+F` is its `git grep` view whose hits open at their line, and `⌥`-click opens a URL in the browser or a `file:line` here. Files open in the vim modal — an in-process PTY running the configured EDITOR over every overlay, which HARDWIRED UNLOCK force-closes. | "find file", "file search", "fuzzy file finder", "grep view", "find in files", "grep", "find in files search", "option click", "alt click", "vim modal", "editor modal", "edit file modal", "edit modal", "the editor", "open in vim", "vim terminal", "a modal inside this app", "file viewer (vim)", "neovim" | `app.rs::Overlay::Files`, `FileFinder` · `f` / `Shift+F` · `nebula-tui/src/vim_term.rs` |
...
| **MEMORY MODAL** | `Shift+M`: the ` Memory ` table (SESSION / WHERE / PID / PROCS / MEM) from the metrics snapshot, re-polled every 2 s, with WARM SPARES under their own `warm spares (N)` tree. `Enter` opens the selected session. | "memory usage", "metrics", "the memory thing", "unknown agents", "metrics modal", "metrics snapshot", "monitor modal" | `app.rs::Overlay::Metrics` · `Shift+M` |
...
| **MODEL / EFFORT** | The per-kind default `--model` and reasoning effort for new sessions (Claude: `--effort`; Codex: `-c model_reasoning_effort=`; Cursor: one flat `--model <family>-<effort>` id — the model is a family such as `claude-opus-5-thinking`, the effort a suffix that family ships — `high`, `high-fast`, bare `fast` — joined by the DAEMON; `fit_effort` swaps a suffix the family lacks, or "default" on a family with no bare id, for the family's fallback), set on the AGENTS TAB, per launch in the NEW SESSION PICKER submenus, or pinned by an AGENT PRESET. "default" always means no flag. | "model", "reasoning effort", "opus / sonnet", "cursor model", "select the model and effort", "cursor catalogue", "cursor models", "current cursor models", "the catalogue", "cursor model family", "family" | `nebula-tui/src/config.rs::CLAUDE_MODELS`, `CODEX_EFFORTS`, `cursor_catalogue.rs::SEED_IDS`, `effort_choices(kind, model)`, `fit_effort` |
...
| **HOOK RECEIVER** | The DAEMON's loopback HTTP server (`127.0.0.1:<random>`, per-boot BEARER TOKEN) that the agents' hook commands `curl`. Unversioned and fail-soft — a PROTOCOL VERSION bump can never break it. | "hook server", "the hooks endpoint", "phone home to nebula", "claude hook dialect", "claude hooks" | `nebula-daemon/src/hooks/mod.rs::start_hook_server` |
...
| TERM | What it is | Also called | Where · key |
|---|---|---|---|
| **PREWARM POOL** | The DAEMON pre-boots one real agent CLI per (WORKTREE, AGENT KIND, MODEL, EFFORT) while you rest on a worktree, and the next `CreateAgent` adopts it so attaching lands on a booted screen. Spares live up to 15 min (`PREWARM_MAX_AGE`) — 150–300 MB each. Opt-out: `"prewarm_agents": false`. | "warm spares", "unknown agents", "prewarmed", "pre-warmed sessions", "sub agents (mistaken)", "prefetch these connections" | `registry.rs::prewarm_agent`, `PrewarmEntry` · `"prewarm_agents"` |
| **WARM SPARE** | One pre-booted CLI in the PREWARM POOL: a real process with a fresh `NEBULA_AGENT_ID` and no store row, hence `warm`, not an agent, in the MEMORY MODAL and FOOTER. Reported from outside as duplicate sessions (issue #15) — they have no SESSIONS PANEL row, so only a process listing shows them. | "spare", "warm", "duplicate sessions", "duplicate claude sessions" | `protocol.rs::PrewarmInfo` |
...
| **IDLE REAPER** | Every 15 s, kill PTYs in WORKTREES no client is viewing once they pass the IDLE TIMEOUT; spares RUNNING and NEEDS FEEDBACK agents and terminals with a command running (the PIN exemption retired with PIN). A reaped agent RESUMES on the next ATTACH. | "the reaper", "idle timeout kills", "auto suspend or kill claude sessions that are not in focus", "reap process", "nebula_idle_reap_ms / nebula_worktree_sync_ms" | `registry.rs::reap_idle_sessions` · `NEBULA_IDLE_REAP_MS` |
...
| **NEBULA RENAME** | Title the current SESSION from inside it — what AUTO-TITLE runs. `--force` retitles a named one; a late attempt is "already titled". | "rename", "the rename hook" | `nebula rename <title> [--force]` · `nebula-tui/src/lib.rs::run_rename` |
| **NEBULA WORKTREE** | Move the current SESSION into a WORKTREE of its PROJECT (WORKTREE RELOCATION); no name invents one, `--base <ref>` picks a new branch's start. | "nebula worktree", "do this in a worktree" | `nebula worktree [name] [--base ref]` · `nebula-tui/src/lib.rs::run_worktree` |
...
| **NEBULA UPGRADE** | Run INSTALL.SH over this binary; refuses a local cargo build without `--force`; a running daemon keeps its old binary until NEBULA KILL. | "upgrade", "update nebula", "nebula_install_dir / nebula_install_url / nebula_upgrade_handoff" | `crates/nebula/src/upgrade.rs` · `nebula upgrade [--force]` |
...
| **SETTING** | One CONFIG.JSON key: `theme`, `animations`, `show_workspaces` (the WORKSPACES BAR), `hide_projects` (the PROJECTS PANEL), `hide_worktrees` (the WORKTREES PANEL), `editor`, `claude_model`/`claude_effort`, `codex_model`/`codex_effort`, `cursor_model`/`cursor_effort`, `claude_enabled`/`codex_enabled`/`cursor_enabled`, `session_idle_timeout`, `done_sound`, `skip_session_naming`, `palette_enter_attaches`, `git_init_on_create`, `close_finder_on_open` (the FILE FINDER closes behind the EDITOR it opens), `keybindings`; daemon-only `prewarm_agents`, `prewarm_sessions`. | "setting", "option", "setting toggle" | `config.rs::Config` |
| **EDITOR** | The editor the FILE FINDER runs (`vim` default; `nvim`, `nano`, `emacs`, `hx`); `NEBULA_EDITOR` overrides. | "editor" | `config.rs::Config::editor` · `NEBULA_EDITOR` |
...
| **VTARGET** | The `CARGO_TARGET_DIR` a RELEASE WORKTREE or SCRATCH WORKTREE builds into — `<scratchpad>/vtarget`, outside the SHARED CHECKOUT's `target/` so a concurrent session cannot thrash its fingerprints. An idle one from another session's scratchpad (untouched 30+ min) can be borrowed for a ~1 min gate; never build two worktrees into the same one across a base check — they share the `nebula` bin's unit hash and the E2E tests run whichever tree compiled last. | "vtarget", "borrowed vtarget", "the release target dir" | `.claude/skills/release/SKILL.md` §2 · `CARGO_TARGET_DIR=<scratchpad>/vtarget` |
| **RELEASE SKILL** | `/release`: verify green, commit, bump `[workspace.package].version`, tag, push, replace the GitHub notes with a real changelog — a one-line opener, benefit-grouped `###` headers with one emoji each, fixes filed under the feature they keep, a `⚠️ Heads up` line for the PROTOCOL VERSION bump, the INSTALL.SH line last (the shape settled 2026-08-28). Triggered by "commit push release". | "commit push release", "commit, push, release", "commit push and release", "commit and push and release", "cut a release", "ship it", "do a release", "make another release" | `.claude/skills/release/SKILL.md` |
...
| **MAKE CYCLE** | The re-runnable full cutover: MAKE INSTALL, then NEBULA KILL (stops the real daemon and every SESSION — run it from a terminal outside nebula), then `make prune` (drops every build hash of a crate but the newest `KEEP=3`, under cargo's build lock — see the Candidates ledger), then MAKE DEV. Install goes first so a failed build kills nothing. | "kill & install & dev all in one", "keep re-running", "the make cycle", "automatically clean up old stuff", "make prune", "clean up old stuff", "prune" | `Makefile::cycle` · `make cycle` |
...
| **PROJECT TERMS** | This file, and the skill that keeps it true after every task: it detects the vocabulary each task surfaced, records aliases, renames and retirements at once, and promotes a new name to a TERM only after it recurs across separate tasks (the Candidates ledger, section 14). | "the glossary", "terms", "the defined terms", "our list of terms", "the terms" | `TERMS.md` · `.claude/skills/project-terms/SKILL.md` |
...
| **RECALL HOOK** | The `UserPromptSubmit` hook that maps the prompt's words onto TERMS (via the Alias index) and file names, scores the MEMORY LOG index lines, and injects the best few entries' Gotchas plus the matching STANDING GOTCHAS as `[nebula recall] …` context. Nebula's own `recall.py`, unrelated to Mission Control's Recall (`.claude/skills/recall`, the `mcp__recall__*` tools). | "nebula recall", "the recall", "recall" | `.claude/hooks/recall.py` · `.claude/settings.json` |
| **GUARD HOOK** | The `PreToolUse` hook on Bash that blocks commands a recurring gotcha warned about (backticks in a double-quoted commit message, `cargo install --path`, an in-place `cp` over `~/.cargo/bin/nebula`, an unquoted `for f in $(…)` in zsh) and feeds the right way back to the agent; heredoc bodies are stripped before matching. | "the guard" | `.claude/hooks/guard.py` |
| **STANDING GOTCHAS** | `.claude/memory/gotchas.md`: the MEMORY LOG's durable traps promoted out of entries, one line each grouped by TERM, with `re-hit ×N` and `retire:` markers; read in full by every session and capped at 300 lines. | "gotchas.md", "standing gotcha", "gotchyas" | `.claude/memory/gotchas.md` |
...
| **SELF-IMPROVING LOOP** | The per-task cycle every AGENT in a nebula SESSION runs: read the MEMORY LOG and PROJECT TERMS → PROMPT DADDY → do the work → NEBULA-MEMORY SKILL → PROJECT TERMS → OUTPUT DOCTOR, so each task leaves the next one better grounded. Its two files are also where a long-lived branch re-conflicts with `main` on every merge. | "the self improving loop", "the self improving look", "our custom shared memory system", "the memory system" | `AGENTS.md` (the protocol) · `CLAUDE.md` (`@AGENTS.md` + the Skill triggers) |
...
| **KEEP MODULES SMALL** | The path-scoped rule for `crates/**/*.rs`: a long file, `impl`, struct or function is a refactoring smell — split what the task already touches into smaller modules, types or functions, behavior-preserving and tested first, no numeric line limit, no drive-by moves of files the task does not touch (the AGENT PRESETS overlays went to `preset_overlays.rs` under it). | "split up larger files", "too long of files is a refactoring smell" | `.claude/rules/rust-modules.md` |
...
| **METRICS SNAPSHOT** | One machine-wide `ps` sweep summed per SESSION process tree (plus the daemon itself and the WARM SPARES), for the MEMORY MODAL and the FOOTER's WARM COUNT. Merged into MEMORY MODAL. Was: `nebula-daemon/src/metrics.rs::collect` · `protocol.rs::MetricsSnapshot` | 2026-08-29 |
| **PENDING ACTION** | What a CONFIRM DIALOG commits on `y`: `DeleteAgent`, `DeleteWorktree`, `RemoveWorkspace{reopen_picker}`, `ResetSettings`, `Quit`, … Merged into CONFIRM DIALOG. Was: `app.rs::PendingAction` | 2026-08-29 |
| **GREP VIEW** | `Shift+F`: `git grep` across the WORKTREE; `Enter` opens the hit at its line. Merged into FILE FINDER. Was: `app.rs::Overlay::Grep` · `nebula-tui/src/grep_search.rs` · `Shift+F` | 2026-08-29 |
| **OPTION CLICK** | `⌥`-click a URL to open it in the browser, or a `file:line` to open it in the VIM MODAL. Merged into FILE FINDER. Was: `event_loop.rs::handle_mouse` · `nebula-tui/src/links.rs` | 2026-08-29 |
| **VIM MODAL** | An in-process PTY running the configured EDITOR over every overlay (from FILE FINDER, its grep view, TREE BROWSER, or an option-click). HARDWIRED UNLOCK force-closes it. Merged into FILE FINDER. Was: `nebula-tui/src/vim_term.rs` · `ui.rs::draw_vim` | 2026-08-29 |
| **HANDSHAKE** | `Hello{protocol_version}` → `HelloOk{daemon_pid}` or `Incompatible`, then `Subscribe` → `Snapshot`. Merged into VERSION SKEW. Was: `protocol.rs` · `nebula-daemon/src/server.rs::handle_client` | 2026-08-29 |
| **CHORD** | A key spelling like `ctrl+shift+f`, `shift+tab`, `cmd+1`, `/`; comma-separated per ACTION ID. Merged into KEYMAP. Was: `keymap.rs::KeyChord::parse` | 2026-08-29 |
| **ACTION ID** | The snake_case name of a KEYMAP action (`focus_left`, `git_diff`, `select_workspace_3`) — the key in `"keybindings"`. Unknown ids are ignored, so stale overrides are harmless. Replaced by KEYMAP. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `keymap.rs::Action::id` | 2026-08-29 |
| **BOOT SWEEP** | On daemon start, agents persisted as live are marked DISCONNECTED. Replaced by AGENT STATUS. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `store.rs::sweep_disconnected` | 2026-08-29 |
| **DISCONNECTED** | ○ — the DAEMON restarted while the agent was live (AGENT STATUS marks these). Replaced by AGENT STATUS. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `AgentStatus::Disconnected` · `store.rs::sweep_disconnected` | 2026-08-29 |
| **TERMINATED** | Magenta ● — the process died mid-run. Replaced by AGENT STATUS. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `AgentStatus::Terminated` | 2026-08-29 |
| **ARCHIVE** | Archiving an AGENT releases its PTY and moves it to the ARCHIVED group; UNARCHIVE brings it back (RESUME on next attach). Replaced by SESSIONS PANEL. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `Action::Archive` / `Unarchive` / `ToggleArchived` · `a` / `u` / `Shift+A` | 2026-08-29 |
| **QUIT** | `q` / `Ctrl+C`: leave the TUI; SESSIONS keep running in the DAEMON. Replaced by TUI. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `Action::Quit` · `q` | 2026-08-29 |
| **ZOOM** | `z`: collapse the sidebars (COLLAPSED) and lock input into the attached session — a full-screen terminal. An ESCAPE HATCH restores the layout. Replaced by LOCKED PANE. Retired 2026-08-29 as a name nobody used; the row above says it. Was: `Action::Zoom` · `app.rs::App::collapsed` · `z` | 2026-08-29 |
| **RAW ATTACH** | Hidden debug client: raw passthrough to a session, `Ctrl+\` detaches. Retired 2026-08-29 as a name nobody used. Was: `nebula _raw-attach [name]` | 2026-08-29 |
...
| **RECALL EVAL** | `make recall-eval` (`.claude/memory/recall_eval.py`, first MAKE CI step after MEMORY CHECK): scores the RECALL HOOK on every entry's own Asked prompt (leave-one-out) and the curated pairs and chatter prompts in `recall_eval.json`, failing on a drop against `recall_baseline.json`. | 2026-08-29 reply ("check in an eval") · 2026-08-29 MEMORY "The RECALL HOOK Gets An Eval, Text Search Over Entries, Ranked STANDING GOTCHAS And A PROMPT DADDY Re-Run" | `.claude/memory/recall_eval.py` · `Makefile::recall-eval` |
| **TERMS CHECK** | `make terms-check` (`.claude/memory/terms_check.py`, third MAKE CI step): reports dead and single-use TERMS, `merge?` pairs, alias collisions, stale `Where` pointers, dangling Alias-index targets, overdue candidates and unmentioned Retired rows; `--merge A --into B`, `--retire A [--into B]` and `--prune` rewrite every reference and re-run RECALL EVAL. | 2026-08-29 prompt ("prune down or merge terms") · 2026-08-29 MEMORY "`terms_check.py` Reports, Merges, Retires And Prunes TERMS.md Rows" | `.claude/memory/terms_check.py` · `Makefile::terms-check` |
| **PROJECT RULE** | A markdown file in `.claude/rules/` with `paths:` frontmatter, loaded only when Claude reads a matching file — where a long instruction that applies to one part of the tree lives instead of `CLAUDE.md`. KEEP MODULES SMALL is the first one. | 2026-08-29 prompt ("follow best practices") · 2026-08-29 MEMORY "`CLAUDE.md` Imports `AGENTS.md` And Names Each Skill's Trigger Instead Of Restating It" | `.claude/rules/rust-modules.md` |
...
| "the bottom bar" (under a tab), "gap under the tab", "the underline" / "green underline" (on a WORKSPACE TAB) | TAB UNDERLINE |
| "the underline" / "green underline" (on a session / worktree / project row) | the PILL ROW's rail (the PROJECT button's `▌`) — the only per-row selection mark |
| "bottom bar", "status bar", "footer" | FOOTER |
| "projects list", "projects column", "projects" | PROJECTS PANEL |
| "archived list", "the groups", "the recent label" | SESSION GROUPS |
| "worktrees column", "worktrees list", "worktrees row" | WORKTREES PANEL |
| "done" (a color / dot / badge / count / sort tier) | UNSEEN — or FINISHED; the split is read-vs-unread |
...
| "needing user input", "waiting on me", "needs feedback" | NEEDS FEEDBACK |
| "green" (a mark, rail, underline or header — on the `forest` THEME) | the THEME accent, not FINISHED; check `theme` in the config first |
...
| "model", "reasoning effort", "opus / sonnet", "cursor model", "select the model and effort" | MODEL / EFFORT |
| "unknown agents", "warm spares", "pre-warmed sessions", "duplicate sessions" | PREWARM POOL / WARM SPARE |
...
| "memory usage", "metrics", "the memory thing", "monitor modal" | MEMORY MODAL |
...
| "fuzzy jump", "slash search", "jump to", "the search", "fuzzy / finder" | PALETTE |
| "type ahead", "type agead" | TYPE-AHEAD |
| "find file" | FILE FINDER |
| "find in files", "grep" | FILE FINDER |
| "file tree", "file browser", "tree view" | TREE BROWSER |
| "editor modal", "edit file modal", "edit modal", "open in vim" | FILE FINDER |
...
| "the socket", "pidfile", "buildstamp" | DAEMON SOCKET / BUILDSTAMP |
| "the ui", "the app", "quit", "q", "closed out" | TUI |
...
| "kill & install & dev all in one", "make cycle", "the make cycle", "automatically clean up old stuff" | MAKE CYCLE |
| "commit push release", "commit, push, release", "commit push and release", "commit and push and release", "cut a release", "ship it", "do a release", "make a release", "make another release" | RELEASE SKILL |
...
| "memory.md", "the memory", "log this", "remember this", "nebula memory", "how nebula memory works", "memories", "older memories" | MEMORY LOG / NEBULA-MEMORY SKILL |
| "gotchas.md", "standing gotcha", "gotchyas" | STANDING GOTCHAS |
| "nebula recall", "the recall", "recall" (not Mission Control's Recall skill / `mcp__recall__*`) | RECALL HOOK |
...
| "the glossary", "terms", "the defined terms", "our list of terms", "the terms" | PROJECT TERMS |
| "the self improving loop", "the self improving look", "our custom shared memory system", "the memory system", "the old memory system approach" | SELF-IMPROVING LOOP |
| "theme", "color scheme" | THEME |
| "panel tint", "focused-panel tint", "focus terminal setting" | FOCUS TINT (always on; no SETTING) |
...
| "recent to top", "order by last interaction", "goes to top of list", "always just move recent to top", "recent at the top", "time stamps", "most recently used" | RECENCY ORDER |
...
| "agent modal", "agent preset modal", "agent definition", "pre-configured agent", "prefix and postfix prompts", "sandwich the request" | AGENT PRESET |
| "the preset form", "presets modal" | PRESET EDITOR |
...
| "family", "cursor model" | MODEL / EFFORT |
| "cursor models", "current cursor models", "the catalogue" | MODEL / EFFORT |
| "make prune", "clean up old stuff", "prune" | MAKE CYCLE |
| "attention order", "sorted to the top by default" | PALETTE |
| "vtarget", "borrowed vtarget", "the release target dir" | VTARGET |
