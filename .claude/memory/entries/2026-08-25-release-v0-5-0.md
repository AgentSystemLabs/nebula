...
**Did:** Followed the RELEASE SKILL (`.claude/skills/release/SKILL.md`) in a private RELEASE WORKTREE. Two commits landed on `main`:
the already-finished-but-uncommitted PILL ROW rail fix + `assets/screenshot.png` (17320c5), then the version
bump to 0.5.0 (1018bdf, tagged `v0.5.0`). All 4 release-matrix targets built and the RELEASE NOTES were rewritten
by hand.
...
- **`origin/main` moved out from under the release mid-task** — PR #12 (`worktree-hl-panel-nav`) merged
  ...
  that: a blind `cp` of the SHARED CHECKOUT's modified file over the fresh `origin/main` copy would have
