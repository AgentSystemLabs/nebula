...
path in `event_loop.rs` make `Tab` toggle `Claude · cloud`; Cloud bypasses the bare-Claude PREWARM POOL and
opens `PromptKind::ClaudeCloudTask`, the CLOUD TASK EDITOR, a wrapped multi-row editor rendered by
...
**Gotchas:**
- A Cloud create must never adopt or refill the normal Claude WARM SPARE: that PTY already started bare
...
- The LOGIN SHELL WRAP puts the task in both its `-c` string and Claude's argv. TUI and daemon both
...
- What works today without a browser: CLOUD TELEPORT, `claude --teleport session_…` (pulls transcript + branch into a
  local session — a snapshot/fork, not a live stream; it refuses a dirty tree with a "Stash changes and
  continue?" prompt, so run it in a fresh worktree) and CLOUD MESSAGE, `claude -p "msg" --cloud session_…` (queue a
