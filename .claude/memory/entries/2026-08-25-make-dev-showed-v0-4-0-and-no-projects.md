...
`origin/main` was at v0.7.0 — every release since had been cut from a private RELEASE WORKTREE and never pulled
back, so `make dev` (MAKE DEV, which builds *this* checkout) faithfully reported 0.4.0. Synced it: `git stash -u`
(kept as `stash@{0}` for safety), `git pull --ff-only`, then restored only the `Makefile` from the stash.
(2) `make dev` runs with `NEBULA_DATA_DIR=~/.nebula-dev`, a deliberately separate DB (the DEV INSTANCE), so it had zero
...
- Every dirty file in the SHARED CHECKOUT except the `Makefile` was either byte-identical to `origin/main` or
...
- Agent rows are only spawned lazily (ENSURE SESSION, `Registry::ensure_session`, `registry.rs:~1857`), so copying
