...
- The user said "move **notes** and links", but NOTES is `e` and never conflicted — the two actions
  actually sitting on `h`/`l` were the **HOSTS PICKER** and NEW LINK. Read the issue, not just the prompt.
- **Never bulk-replace `Char('h')`/`Char('l')` in `event_loop.rs`.** Most hits are overlay-local grammar
  the KEYMAP doesn't own and must not change: SETTINGS OVERLAY tab/value cycling (~3495-3514), the DIFF VIEWER and TREE
  BROWSER (~2919, ~2960-2966). Only the *test* presses needed swapping.
- FOOTER and HELP OVERLAY hints come from `Keymap::first(action)`, so **the chord you want displayed has to lead
