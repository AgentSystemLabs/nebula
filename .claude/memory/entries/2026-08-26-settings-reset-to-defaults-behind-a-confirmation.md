...
**Did:** `Shift+R` anywhere in the SETTINGS OVERLAY (tab strip or list) swaps in a CONFIRM DIALOG (`ConfirmDialog`) with
...
- **`Config::save()` is a patch, not a write — it can't reset.** It merges the TUI's known keys into
  ...
  difference.
- **The SETTINGS OVERLAY's inner width is 82 columns and `settings_keys_hint` is `truncate`d to it** (with
