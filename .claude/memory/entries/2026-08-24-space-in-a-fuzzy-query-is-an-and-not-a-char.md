...
**Did:** FUZZY MATCH (`crates/nebula-tui/src/fuzzy.rs::fuzzy_match`) now splits the query on whitespace and requires
...
`query.split_whitespace().next().is_none()`. One matcher change covers all four call sites (the `/` PALETTE,
the DIFF VIEWER file filter, the `f` FILE FINDER, the TREE BROWSER at `tree_browser.rs:300`). 8 unit tests in `fuzzy.rs` plus
