...
**Did:** Added the PROGRESS SCANNER, `crates/nebula-daemon/src/pty/progress.rs`, which scans the PTY byte stream for OSC 9;4
progress edges; the pump emits `PtyEvent::Progress` and the STATUS MACHINE (`status.rs`) treats "progress cleared" as a
synthetic `Stop` (same STOP GATE subagent-drain bookkeeping), but only from Running/NeedsFeedback.

**Gotchas:**
- Esc-cancelling a Claude turn fires **no HOOK EVENT at all**. `Stop` is documented not to run on user
  interrupt, and the IDLE PROMPT (`idle_prompt` Notification) that normally rescues a hookless turn end is suppressed
