...
**Did:** Ctrl+Q is the unlock/ESCAPE HATCH (the HARDWIRED UNLOCK). Fallbacks kept: Ctrl+] / Ctrl+Esc / Ctrl+←. Shift-drag was
replaced with app-side plain DRAG SELECT in the TERMINAL PANE (REVERSED overlay for highlight, text via
...
- **The user runs Terminal.app**, not Ghostty, despite Ghostty being installed. Terminal.app fails the
  KITTY PROTOCOL probe, so Cmd-modified keys and Ctrl+Esc never reach the app there.
