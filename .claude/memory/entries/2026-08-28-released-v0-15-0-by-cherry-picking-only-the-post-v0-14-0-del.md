...
**Did:** Cut **v0.15.0** (0.14.0 → minor: new user-facing features). Carried the PR SESSION launch (`CreatePrAgent`, MIGRATION 22 `agents.pr_url`), the NEW LINK removal / WORKTREE OPEN PRS GROUP,
NEBULA TUNNEL's ttyd reuse, and the PROMPT DADDY / OUTPUT DOCTOR skill revisions. RELEASE WORKTREE on
...
tag, all four matrix targets green, RELEASE NOTES via `gh release edit --notes-file`. Then — the "pull latest"
...
**Gotchas:**
- **The SHARED CHECKOUT was 30 commits / 1 release behind, and its uncommitted diff was two layers**: the
