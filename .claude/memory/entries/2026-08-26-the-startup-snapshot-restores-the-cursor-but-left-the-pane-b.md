...
UI STATE BLOB (`UiState`) and then stopped — nothing ever sent the `Attach`. `restore_ui_state` now returns `bool`
("the remembered `session_agent` landed under the cursor") and the Snapshot arm calls `preview_selected`
when it does, so the TERMINAL PANE comes back with the row, FOCUS staying on the panels exactly like a cursor
...
**Gotchas:**
- `restore_context` / `restore_session` (SELECTION MEMORY) can't be reused on the startup path: they read
