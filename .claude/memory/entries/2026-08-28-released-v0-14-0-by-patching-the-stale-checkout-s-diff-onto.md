...
**Did:** Cut **v0.14.0** (0.13.0 → minor: new user-facing features). `release` skill in a RELEASE
...
`git push origin release-v0.14.0:main`, tag, all four matrix targets green, RELEASE NOTES replaced via
...
- **The SHARED CHECKOUT was 8 commits behind `origin/main` again — this time a merged PR (#17), not a
  ...
  Better recipe than the skill's cp: in the SHARED CHECKOUT `git diff HEAD > local.patch`, in the RELEASE WORKTREE
...
- The one conflict was a signature skew: PR #17 gave `enter_terminal_pane` an `out: &mut Vec<ClientRequest>`
  ...
  sites in `handle_key`. The SHARED CHECKOUT's uncommitted `event_loop.rs` is therefore *not* what shipped —
