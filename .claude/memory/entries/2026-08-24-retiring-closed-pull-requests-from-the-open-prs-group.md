...
Refresh Every 15 s And On Focus") in `crates/nebula-tui/src/event_loop.rs` (that GIT POLL beat *is* the
...
PR PREVIEW detail fetch comes back `MERGED`/`CLOSED`, ahead of the next list. Drafts needed **no code change**
...
- `gh pr list --state open` includes drafts by construction — do not "fix" a missing draft by adding a
  filter. If drafts ever go missing the bug is downstream, in `parse_list` or the DRAFT BADGE (`is_draft`) at
  `ui.rs:2615`.
