...
**Did:** `7570387` re-homes an AGENT row by hook-reported cwd (CWD REPARENT). The cwd probe is the
...
- A Bash `cd` to a directory **outside the session's workspace root is silently reset** ("Shell cwd was
  reset to …") and the hook cwd never changes. So nebula's own WORKTREE DIR layout
...
- **Hooks are snapshotted at session start**, so any MANAGED HOOKS change only reaches newly spawned SESSIONS.
