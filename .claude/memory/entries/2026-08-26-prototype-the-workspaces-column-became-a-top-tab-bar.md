...
**Did:** Replaced `draw_workspaces` (the 18-wide WORKSPACES COLUMN) with `draw_workspaces_bar`
(`crates/nebula-tui/src/ui.rs:~2320`), the WORKSPACES BAR, a 3-row strip across the top of the body: blank spacer, then
`   WORKSPACES · n` plus one WORKSPACE TAB per workspace, then a full-width rule **broken under the open tab** so it
...
` n done`, the workspace's UNSEEN finishes; see [Done Reads Violet And Says "done"] — and the bar scrolls
...
Keys: SELECT WORKSPACE N — new `Action::SelectWorkspace(u8)` with nine `workspace_slot!` `ACTIONS` rows
...
steps out to Projects, `↑` no-ops; the PANEL WALK (`Tab`/`Shift+Tab`) still includes the bar, `←` from Projects no longer
...
**Gotchas:**
- **`⌘1`–`⌘9` cannot work in the user's terminal and never will.** `keymap::host_warning` (the HOST REACH WARNING) already returns
...
- **An empty non-default workspace still shows the SPLASH on `origin/main`.** The
  ...
  switch to.
- The SHARED CHECKOUT was dirty from other sessions again ([Shared tree races]) while `HEAD...origin/main` read
