...
**Did:** Cut **v0.13.0** (0.12.0 → minor: new user-facing features). RELEASE SKILL (`release`) as written: private
RELEASE WORKTREE on `release-v0.13.0` off `origin/main`, twelve files copied in by content,
...
`git push origin release-v0.13.0:main`, tag, all four matrix targets green, RELEASE NOTES replaced via
`gh release edit`. Carried the OSC 52 CLIPBOARD ROUTE, the CLOUD MIRROR work, and the TAB UNDERLINE
...
- **The SHARED CHECKOUT was 7 commits / 2 releases behind `origin/main`** (local `main` at
  ...
  cut from a worktree and never fast-forwards the SHARED CHECKOUT — **assume it, don't check for it**.
