...
**Did:** Nothing to pull — local `main` already tracked `origin/main` at `8beee5a` (the SHARED CHECKOUT was
reconciled between releases), so no merge was needed and the RELEASE SKILL's plain recipe applied:
RELEASE WORKTREE cut from `origin/main`, the six dirty files copied in by content, `274ece8` (feature), `db718af`
(memory), `199011c` (bump to 0.8.0, tagged `v0.8.0`). 624 tests green, all 4 matrix targets built, RELEASE NOTES
...
tab focus, snapshot re-attach, PROJECT DIVIDERS removal, and the DEFAULT WORKSPACE SPLASH gate (entries above).
...
- Two clean releases in a row now. The SHARED CHECKOUT is left dirty-but-identical after each one (the
...
- `cargo clippy --workspace --all-targets` carried 8 warnings at v0.10.0 (unneeded `return`, items after a
  ...
  blocker: the only workflow is the RELEASE WORKFLOW (`.github/workflows/release.yml`) and it runs no clippy or fmt step. Report
