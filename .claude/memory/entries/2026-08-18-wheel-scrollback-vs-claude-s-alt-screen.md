...
**Did:** WHEEL SCROLL — `handle_mouse` in `event_loop.rs` (see `mouse_protocol_mode` at `event_loop.rs:5199`) now
forwards a real SGR wheel report (`\x1b[<64;col;rowM` / 65) at the 1-based TERMINAL PANE cell whenever the MOUSE MODE
...
- The old arrow-synthesis fallback is what triggered Claude's own `arrow-burst` detector and that warning
  banner. Check the child's MOUSE MODE in the VENDORED VT100 before assuming arrows are right.
