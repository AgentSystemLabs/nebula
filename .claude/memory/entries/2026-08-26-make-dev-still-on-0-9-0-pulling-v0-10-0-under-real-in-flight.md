...
`origin/main` was `0361f0a` / v0.10.0 (PR #16, the WORKSPACES BAR merge). The difference this time: the dirty
tree was not stale leftovers but ~1,300 lines of *uncommitted, un-branched* work (the three entries just
under this one: CLOUD ATTACH, workspace-delete CONFIRM DIALOG, UNSEEN badges) based on `249668e`, overlapping
almost every incoming file. Recipe that worked, in order: `git stash create` (a stash commit without
touching the tree) → `git worktree add --detach <scratch> <that sha>` (a SCRATCH WORKTREE) → `git merge origin/main` there →
...
**Gotchas:**
- `git stash create` ignores untracked files, so the SCRATCH WORKTREE merge failed with `E0583 file not found for
...
- Conflict shape when the WIP already contains part of the incoming range: hunks where origin/main's later
  ...
  only files the WORKSPACES BAR prototype (`30042e9`) rewrote needed thought — `leftmost_focus` → `first_focus`
...
- `workspace_scope_is_per_connection` (E2E PTY) failed twice under a parallel clippy+test run and passed
  alone: the Ack-beats-upsert load race the v0.10.0 entry describes, not the merge (test fixed
  2026-08-27, `6638952`).
- The old Makefile's dev daemon lives at `/tmp/nebula-dev`; the new per-checkout DEV INSTANCE slot is
