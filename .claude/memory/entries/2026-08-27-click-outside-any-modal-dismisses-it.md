...
**Did:** Only four OVERLAYS lacked it — the PALETTE, FILE FINDER, GREP VIEW, TREE BROWSER, HOSTS PICKER, SETTINGS OVERLAY, MEMORY MODAL and the
...
back in the SETTINGS OVERLAY, the WORKSPACE SWITCHER delete reopens the switcher, and an abandoned Claude name
prompt still restores the WARM SPARE's spec. `Overlay::Help` became `Help(HelpView { area })`;
...
**Gotchas:**
- The DIFF VIEWER's Esc is two-stage (clears the file filter first), so click-outside must close it directly rather
